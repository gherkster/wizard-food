<template>
  <div>
    <v-row v-for="(item, index) in items" :key="item.uuid">
      <v-text-field v-if="item.itemType === 'section'" label="Section" v-model="item.label" :rules="[rules.required('Section')]" />
      <template v-else>
        <v-text-field
          v-if="hasAmount"
          label="Amount"
          v-model="item.amount"
          :rules="[rules.required('Amount'), rules.isDecimal('Amount')]"
        />
